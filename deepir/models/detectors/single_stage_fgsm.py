...
class SingleStageDetectorFGSM(BaseDetector):
    ...
    def __init__(self,
                 ...
                 fgsm_epsilon=0.01,
                 back_rate = 0.01):
        super(SingleStageDetectorFGSM, self).__init__(init_cfg)
        if pretrained:
    ...
    def forward_train(self,
        ...
        """
        super(SingleStageDetectorFGSM, self).forward_train(img, img_metas)
        ### AAL MODIFIED ###
        adv_delta = torch.zeros_like(img).to(img.device).requires_grad_()
        # 1. First forward: clean inference
        feats, feats_extra, _ = self.extract_feat(img+adv_delta)
        feats = list(feats)
        feats.extend(feats_extra)
        ...
        # 2. Second forward: adv train
        loss_sum.backward()
        adv_delta_grad = adv_delta.grad
        adv_delta = (self.fgsm_epsilon * torch.sign(adv_delta_grad)).detach()
        feats, feats_extra, _ = self.extract_feat(img+adv_delta)
        feats = list(feats)
        feats.extend(feats_extra)
        losses = self.bbox_head.forward_train(feats, img_metas, gt_bboxes,
